...
func main() {
	...
	im, err := imap.New("penguinacity", "imap_password", "imap.gmail.com", 993)
	check(err)
	defer im.Close()
	...
	for _, f := range folders {
		...
		//		uids, err := im.GetUIDs("FROM do-not-reply@voipwelcome.com")
		uids, err := im.GetUIDs("FROM inmail-hit-reply@linkedin.com")
